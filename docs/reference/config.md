...
| `ws_media_relay` | bool | `false` | `true` or `false` | **Primary browser media plane.** Relay browser media datagrams (binary WebSocket Cadence frames, client-held E2EE ciphertext) between a channel's call participants. Off by default; opt-in. When on, `MEDIA JOIN` hands each WS participant a per-stream MAC key as `EVENT <nick> MEDIA MACKEY <#chan> <base64>`, and each datagram is lenient-verified before fan-out to same-node call members (including cross-shard). The daemon never decrypts E2EE payloads and never rewraps them onto native UDP or WebRTC RTP; WS OFFER suppresses unroutable NATIVE/TRANSPORT candidates (`src/daemon/config_format.zig:466`, `src/daemon/config_boot.zig:102`, `src/daemon/server.zig` `handleWsMediaDatagram` / `provisionMediaTransports`, `src/substrate/cadence_frame.zig`). |
| `ws_media_require_mac` | bool | `false` | `true` or `false` | Require a valid per-stream MAC tag on every browser media datagram. `false` relays untagged datagrams (a present tag must still verify); `true` drops untagged/bad-tag datagrams. Same `native_stream_key`-derived `(channel, participant)` key as the native leg. Prefer `true` with `ws_media_relay` in production (`src/daemon/config_format.zig:470`, `src/daemon/config_boot.zig:103`). |
